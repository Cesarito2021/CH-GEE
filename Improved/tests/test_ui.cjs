...
assert.equal(Object.keys(ctx.recipes).length,3);assert.equal(ctx.clouds.o.min,0);assert.equal(ctx.clouds.o.max,100);
ctx.dataset.setValue('Pred 2 · S1 + S2 + COP + XY');ctx.updateRecipe();assert.equal(ctx.clouds.fieldGroup.style().get('shown'),true);
ctx.dataset.setValue('Pred 3 · AlphaEarth + COP + XY');ctx.updateRecipe();assert.equal(ctx.clouds.fieldGroup.style().get('shown'),false);
...
console.log('PASS: one map, three predictors, original dispatch, slider, cache, palette and stale-response protection.');
